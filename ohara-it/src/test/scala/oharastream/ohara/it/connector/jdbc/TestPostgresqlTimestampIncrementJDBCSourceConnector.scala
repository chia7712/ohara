...
class TestPostgresqlTimestampIncrementJDBCSourceConnector extends TestPostgresqlTimestampJDBCSourceConnector {
  override protected[jdbc] val props: JDBCSourceConnectorConfig = JDBCSourceConnectorConfig(
    TaskSetting.of(
      Map(
        ...
        "source.increment.column.name" -> incrementColumn
      ).asJava
    )
  )
}
